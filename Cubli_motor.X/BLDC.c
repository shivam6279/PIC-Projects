    ...
    for(i = 0; i < ENCODER_RES; i++) {
        for(j = 0; j < len; j++) {
            ...
            if(i >= arr[j][0] && i < arr[k][0]) {
                delta = i - arr[j][0];
                delta = delta / (arr[k][0] - arr[j][0]) * (arr[k][1] - arr[j][1]);
                break;
                ...
                delta = delta / (arr[k][0] - arr[j][0] + ENCODER_RES) * (arr[k][1] - arr[j][1]);
                break;
            }
        }
