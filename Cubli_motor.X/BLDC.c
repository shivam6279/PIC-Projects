...
double encoder_calib_data[(int)POLE_PAIRS*6] = {0.0000, 4.8339, 9.4042, 13.7109, 19.4238, 23.3789, 28.9160, 33.5742, 38.2324, 42.9785, 48.7792, 52.6464, 58.3593, 63.5449, 68.2910, 73.0371, 79.2773, 83.7597, 89.6484, 94.6582, 99.5800, 104.6777, 110.6542, 114.6093, 120.4101, 125.6835, 130.5175, 135.0878, 140.9765, 145.1953, 150.6445, 155.5664, 160.0488, 164.8828, 170.4199, 174.1992, 179.6484, 184.5703, 188.9648, 193.4472, 199.1601, 203.3789, 208.7402, 213.5742, 218.1445, 223.0664, 228.6035, 232.5585, 238.2714, 243.3691, 247.9394, 252.7734, 258.8378, 263.2324, 269.1210, 274.3945, 279.3164, 284.5898, 290.5664, 294.6972, 300.5859, 305.8593, 310.6054, 315.2636, 321.4160, 325.5468, 331.2597, 336.0058, 340.5761, 345.2343, 350.8593, 354.4628};
...
//    position = pos_cnt / ENCODER_RES * 360.0 - motor_zero_angle;
...
void interpolate_encoder_lut(unsigned int len, double in[]) {
    int i, j, k;
    double delta_index = (float)ENCODER_RES / (float)len;
    double delta;
    ...
    delta_index = (float)ENCODER_RES / (float)len;
    for(i = 0; i < ENCODER_RES; i++) {
        min_delta = 10000;
        for(j = 0; j < len; j++) {
            k = (j + 1) % len;
            if(i > arr[j][0] && i < arr[k][0]) {
                min_delta = i - arr[j][0];
                delta = min_delta / (arr[k][0] - arr[j][0]) * ENCODER_RES/len;
                break;
            } else if((fabs(arr[k][0] - arr[j][0]) > ENCODER_RES/2) && (i > arr[j][0] || i < arr[k][0])) {
                min_delta = i - arr[j][0];
                if(min_delta > ENCODER_RES/2) {
                    min_delta -= ENCODER_RES;
                } else if(min_delta < -ENCODER_RES/2) {
                    min_delta += ENCODER_RES;
                }
                delta = min_delta / (arr[k][0] - arr[j][0] + ENCODER_RES) * ENCODER_RES/len;
                break;
            }
        }
