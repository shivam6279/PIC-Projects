    ...
    while(1) {
        if (A1339_read(0x20, &angle) == kNOERROR) {
            if (CalculateParity(angle)) {
                USART4_send_str("Angle = ");
                USART4_write_int((float)(angle & 0x0FFF) * 360.0 / 4096.0);
                USART4_send_str(" Degrees\n");
            } else {
                USART4_send_str("Parity error on Angle read\n");
            }
        } else {
            USART4_send_str("Unable to read Angle\n");
        }

        if (A1339_read(0x28, &temperature) == kNOERROR) {
            USART4_send_str("Temperature = ");
            USART4_write_float(((float)(temperature & 0x0FFF) / 8.0) + 25.0, 2);
            USART4_send_str(" C\n");
        } else {
            USART4_send_str("Unable to read Temperature\n");
        }

        if (A1339_read(0x2A, &fieldStrength) == kNOERROR) {
            USART4_send_str("Field Strength = ");
            USART4_write_int(fieldStrength & 0x0FFF);
            USART4_send_str(" Gauss\n");
        } else {
            USART4_send_str("Unable to read Field Strength\n");
        }
        delay_ms(500);
    }
}
