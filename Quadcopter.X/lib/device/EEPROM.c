...
bool eeprom_writeByte(unsigned char addr, unsigned char byte) {    
    return I2C_WriteRegisters(EEPROM_ADDRESS, (unsigned char[2]){addr, byte}, 2);
}
...
bool eeprom_readBytes(unsigned char addr, unsigned char *bytes, unsigned char num) {
    return I2C_ReadRegisters(EEPROM_ADDRESS, addr, bytes, num);
}
