...
volatile unsigned long int gyro_aq_counter = 0, acc_aq_counter = 0, compass_aq_counter = 0, baro_aq_counter = 0;
...
void PIDSetDiff(PID *x, int diff) {
    x->diff_sign = diff;
}
...
void PIDIntegrateAngle(PID *a, float deltat) {    
    if(a->integral_max_diff <= 0 || fabs(LimitAngle(a->error - a->offset)) <= a->integral_max_diff) {   
        a->integral += LimitAngle(PIDOffsetDiff(*a)) * deltat;
    }
...
void PIDOutput(PID *a) {
    a->output = a->kp * (PIDOffsetDiff(*a)) + a->ki * a->integral + a->kd * a->derivative;
}

void PIDOutputAngle(PID *a) {
    a->output = a->kp * LimitAngle(PIDOffsetDiff(*a)) + a->ki * a->integral + a->kd * a->derivative;
}
