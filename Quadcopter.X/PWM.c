...
void pwm_init(float freq){
    ...
    I2C_WriteRegisters(PCA9685_ADDR, (unsigned char[2]){0, 16}, 2);
    I2C_WriteRegisters(PCA9685_ADDR, (unsigned char[2]){0xFE, prescale}, 2);
    I2C_WriteRegisters(PCA9685_ADDR, (unsigned char[2]){0, 0}, 2);
    while(t-- > 0);
    I2C_WriteRegisters(PCA9685_ADDR, (unsigned char[2]){0, 0xA1}, 2);
}

void set_pwm(int num, int on, int off){
    I2C_WriteRegisters(PCA9685_ADDR, (unsigned char[5]){(0x06 + 4 * num), (on & 0xFF), ((on >> 8) & 0x1F), (off & 0xFF), ((off >> 8) & 0x1F)}, 5);
}
